...
export async function POST(
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      console.log(`Session status: ${chatSession.status}, current agent: ${chatSession.agentId || 'none'}`);
      ...
      if (chatSession.status !== "waiting" || chatSession.agentId) {
        ...
        if (chatSession.agentId === agentId) {
          console.log(`Session ${sessionId} already claimed by this agent ${agentId}`);
          return { 
            success: true, 
            alreadyClaimed: true, 
        ...
        console.log(`Session ${sessionId} already claimed by another agent: ${chatSession.agentId}`);
        throw new Error(
          `Session cannot be claimed (current status: ${chatSession.status}, ${
      ...
      console.log(`Updating session ${sessionId} to assign to agent ${agentId}`);
